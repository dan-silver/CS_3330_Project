...
public class ATCUI_impl extends JFrame implements ATCUI, ActionListener,
	...
	public void PlaneNew(Plane p) {
		...
		infoArea.validate();
		if (ATC.debug_flag)
			System.out.println("p.n.3"); // DEBUG
		synchronized (this) {
			planes.put((Object) (new Character(id)), (Object) uiplane);
		}

		if (ATC.debug_flag)
			System.out.println("p.n.4"); // DEBUG
		PlaneUpdate(p);
		if (ATC.debug_flag)
	...
	public void PlaneUpdate(Plane p) {
		...
		if (ATC.debug_flag)
			System.out.println("p.u.1 il=" + uiplane.info_label); // DEBUG
		if (uiplane == null)
			return;
		if (ATC.debug_flag)
			System.out.println("p.u.2"); // DEBUG
		...
		if (p.takeoff_flag) {
			if (uiplane.radar_label == null) {
				...
				if (ATC.debug_flag)
					System.out.println("p.u.4"); // DEBUG
			} else {
				if (ATC.debug_flag)
				...
				if (ATC.debug_flag)
					System.out.println("p.u.6"); // DEBUG
			}
